...
int main(int argc, char** argv) {
    if (argc < 4) {
        std::cout << "Usage: pglnet_onnxruntime <model.onnx> <input.jpg> <output.jpg> [runs=20] [resize_back=0|1]\n";
        return 1;
    }
    ...
    if (showInput.size() != output.size()) {
        cv::resize(showInput, showInput, output.size());
    }
    cv::Mat compare;
