...
def main():
    parser = argparse.ArgumentParser(description="PGLNet OpenVINO inference")
    parser.add_argument("--model", "-m", required=True, help="Path to OpenVINO .xml")
    parser.add_argument("--input", "-i", required=True, help="Path to input image")
    parser.add_argument("--output", "-o", default="output.jpg", help="Path to output image")
    parser.add_argument("--device", "-d", default="CPU", choices=["CPU", "GPU", "AUTO"])
    ...
    if args.threads is not None:
        config["INFERENCE_NUM_THREADS"] = str(args.threads)
    ...
    height = args.height if shape[2].is_dynamic else int(shape[2])
    width = args.width if shape[3].is_dynamic else int(shape[3])
    input_dtype = np.float16 if input_layer.get_element_type() == ov.Type.f16 else np.float32
    ...
    print(f"Input: {input_layer.any_name} {list(shape)} {input_layer.get_element_type()}")
    print(f"Output: {output_layer.any_name} {list(output_layer.shape)} {output_layer.get_element_type()}")
    print(f"Tensor: {input_tensor.shape} {input_tensor.dtype}")

    for _ in range(args.warmup):
        _ = compiled([input_tensor])[output_layer]

    times = []
    output = None
    for _ in range(args.runs):
        t0 = time.perf_counter()
        output = compiled([input_tensor])[output_layer]
